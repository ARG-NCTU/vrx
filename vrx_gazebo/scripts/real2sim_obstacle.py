...
class RealtoSimObstacle:
    def __init__(self):
        ...
        self.timer_set_model_state = rospy.Timer(rospy.Duration(0.01), self.timer_callback)
    ...
    def rm_original_obstacles(self, first_point, last_point):
        ...
        elif left_boundary <= x1 <= right_boundary and \
            bottom_boundary <= y1 <= top_boundary and \
            left_boundary <= x2 <= right_boundary and \
            bottom_boundary <= y2 <= top_boundary :
            return False    
