...
class Node():
    ...
    def cbJoy(self,data):
        ...
        if not self.auto:
            self.left_msg.data = ((-1)*data.axes[0]+data.axes[1]+(-1)*data.axes[3])*self.linear_scaling
            self.right_msg.data = (data.axes[0]+data.axes[1]+data.axes[3])*self.linear_scaling 
            self.left_lateral_msg = (data.axes[0]+data.axes[1]+(-1)*data.axes[3])*self.angular_scaling
            self.right_lateral_msg = ((-1)*data.axes[0]+data.axes[1]+data.axes[3])*self.angular_scaling
